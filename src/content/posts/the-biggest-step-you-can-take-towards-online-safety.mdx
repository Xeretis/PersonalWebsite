...
If you want to check, whether you have been affected by a data breach, then check out [have i been pwned](https://haveibeenpwned.com/).
...
If you let Chrome or Safari generate your passwords and then **trust your browser to store them, you are basically doing the same thing as described above**. Google doesn't even use [E2EE](https://en.wikipedia.org/wiki/End-to-end_encryption) by default, so **the encryption keys can get leaked alongside your passwords** in a data breach. If someone gets access to your device where these passwords are stored, they can also **quite easily extract them from the browser**.
